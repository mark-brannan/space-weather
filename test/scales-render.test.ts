 ...
 *
 * No flare fixture is paired in: none of these captures has one, and the
 * product treats a failure on that endpoint as best-effort.
 */
const publishedFrom = (name: string) => publishedScalesTree(name)
 ...
 *
 * Deliberately not `NoaaScaleNames` from `src/parse.ts`: the whole point of
 * this block is an expectation sourced outside the codebase, and reading the
 * plugin's own vocabulary table would make a typo in it self-confirming.
 */
